...
Let's use 5 folds for this vignette (and run in parallel with 2 cores, if available):
...
results.cv <- crossValWithGs3(
    ...
    nb.folds = 5,
    seed = 123,
    remove.files = "none", # use "some" in real-life applications
    nb.cores = 2,
    verbose = 2)
```
...
```{r}
results.cv
apply(results.cv[,c("rmspe","cor.p","reg.intercept","reg.slope")], 2, function(x){
